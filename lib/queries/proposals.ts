...
export async function getProposalById(id: string): Promise<SwapProposal | null> {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      proposer:profiles!proposer_id(name, handle, avatar_url, rating, review_count),
      receiver:profiles!receiver_id(name, handle, avatar_url, rating, review_count),
      swap_proposal_items(
        proposal_id, listing_id, side, created_at,
